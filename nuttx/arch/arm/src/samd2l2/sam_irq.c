...
void up_irqinitialize(void)
{
  ...
  sam_eic_initialize(BOARD_GCLK_EIC);
 #endif
...
void sam_dumpnvic(const char *msg, int irq)
{
  ...
  irqinfo("SYSCON:\n");
  irqinfo("  CPUID:      %08x\n",
          (unsigned int)getreg32(ARMV6M_SYSCON_CPUID));

  irqinfo("  ICSR:       %08x AIRCR:  %08x\n",
          (unsigned int)getreg32(ARMV6M_SYSCON_ICSR),
          (unsigned int)getreg32(ARMV6M_SYSCON_AIRCR));

  irqinfo("  SCR:        %08x CCR:    %08x\n",
          (unsigned int)getreg32(ARMV6M_SYSCON_SCR),
          (unsigned int)getreg32(ARMV6M_SYSCON_CCR));

  irqinfo("  SHPR2:      %08x SHPR3:  %08x\n",
          (unsigned int)getreg32(ARMV6M_SYSCON_SHPR2),
          (unsigned int)getreg32(ARMV6M_SYSCON_SHPR3));
