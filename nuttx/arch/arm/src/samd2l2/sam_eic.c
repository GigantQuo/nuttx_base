...
static int sam_eic_isr(int irq, void *context, void *arg)
{
  ...
  for (bit = 0; bit < SAM_IRQ_NEXTINTS; bit++)
  {
    if (intflag >> bit & 0x1)
    {
      irq_dispatch((int)((char)SAM_IRQ_EXTINT0 + (char)bit), context);
      break;
    }
  }
...
/****************************************************************************
 * Name: sam_eic_irq_disable
 *
 * Description:
 *   Disable an external interrupt.
 *
 * Input Parameters:
 *   irq - SAM_IRQ_EXTINTn IRQ to be disabled
 *
 * Returned Value:
 *   None
 *
...
int sam_eic_irq_disable(int irq)
{
  int eirq = irq - SAM_IRQ_EXTINT0;
...
int sam_eic_config(uint8_t eirq, port_pinset_t pinset)
{
  ...
  val = 0;
  type = getpart(pinset, PORT_INT_MASK);
  ...
  if (eirq < 8)
  {
    ...
    if (type == getpart(PORT_INT_RISING, PORT_INT_MASK))
    {
      val = EIC_CONFIG0_SENSE_RISE(eirq);
    }

    if (type == getpart(PORT_INT_FALLING, PORT_INT_MASK))
    {
      val = EIC_CONFIG0_SENSE_FALL(eirq);
    }

    if (type == getpart(PORT_INT_BOTH, PORT_INT_MASK))
    {
      val = EIC_CONFIG0_SENSE_BOTH(eirq);
    }

    if (type == getpart(PORT_INT_HIGH, PORT_INT_MASK))
    {
      val = EIC_CONFIG0_SENSE_HIGH(eirq);
    }

    if (type == getpart(PORT_INT_LOW, PORT_INT_MASK))
    {
      val = EIC_CONFIG0_SENSE_LOW(eirq);
    }
  }
  else if (eirq < 16)
  {
    ...
    if (type == getpart(PORT_INT_LOW, PORT_INT_MASK))
    {
      val = EIC_CONFIG1_SENSE_LOW(eirq);
    }
  ...
  else
  {
    ...
    if (type == getpart(PORT_INT_HIGH, PORT_INT_MASK))
    {
      val = EIC_CONFIG2_SENSE_HIGH(eirq);
    }

    if (type == getpart(PORT_INT_LOW, PORT_INT_MASK))
    {
      val = EIC_CONFIG2_SENSE_LOW(eirq);
    }
